...
export class
   ...
   */
    t
      ...
      // Calculate overall score based o
        categoryBreakdown,
      ...
      throw new Error('Failed to complete skill
  }
  /**
   */
    const prompt = sp
    ...
    - Proficiency le
    - Demand trend (rising/stable/de
    - Learning difficulty
    - Related skills
    - Industry demand breakdown
    - Learning resources
    - Companies known for this skill
    Format as JSON with categories array containing: ca

    const data = JSON.parse(response)
    retu

   * Identify missing skills that would boost career 
  private async identifySkillGaps(profileData: ProfileD
    Based on this professi
    Industry: ${profileD
  ...
  private async identifySkillGaps(profileData: ProfileData): Promise<SkillGap[]> {
    ...
    time to acquire, learning path steps, estimated cost, and ROI score.
  ...
  private calculateOverallScore(analysis: {
    ...
    const categoryScore = analysis.categoryBreakdown.reduce((sum, cat) => sum + cat.categoryScore, 0) / analysis.categoryBreakdown.length
    const alignmentScore = analysis.marketAlignment.alignmentScore
    const competitiveScore = analysis.competitiveAdvantage.competitiveScore
    const trendScore = analysis.skillTrends.reduce((sum, trend) => sum + Math.max(0, trend.trendScore + 10) / 2, 0) / analysis.skillTrends.length
...
export const skillsAnalysisService = new SkillsAnalysisService()    Identify skill gaps in categories like:
    - Technical skills trending in the industry
    - Leadership/management skills for career progression
    - Cross-functional skills for better collaboration
    - Emerging technologies relevant to their field
